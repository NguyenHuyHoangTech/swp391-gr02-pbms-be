...
/**
 * DashboardService đóng vai trò tổng hợp các số liệu thống kê trực tiếp (Live Data)
 * để hiển thị lên bảng điều khiển (Dashboard) của Admin.
 * Service này sử dụng trực tiếp JdbcTemplate (Native SQL) thay vì JPA/Hibernate
 * để tối ưu tốc độ tính toán các câu lệnh đếm phức tạp.
 */
@Service
...
public class DashboardService {
    ...
    public Map<String, Object> getOperationalOverview(LocalDate date) {
        if (date == null)
            // [BỔ SUNG MỚI]: Sử dụng TimeProvider để đồng bộ múi giờ / mock thời gian test thay vì LocalDate.now()
