...
public class PricingConfigurationService {
    ...
    /**
     * Lấy danh sách toàn bộ các chính sách giá đang có trong hệ thống.
     * Chuyển đổi từ các entity sang đối tượng DTO để trả về cho Front-end.
     *
     * @return Danh sách các chính sách giá.
     */
    ...
    /**
     * Lưu mới hoặc cập nhật một chính sách giá.
     * Quá trình này sẽ sử dụng Orphan Removal để xóa toàn bộ các Cấu hình Ca (Shift) và Block cũ.
     * Sau đó tái tạo (insert) toàn bộ Shift và Block mới theo DTO truyền lên.
     * Việc xóa đi tạo lại nhằm đảm bảo tính nhất quán dữ liệu cấu hình phức tạp.
     * Đồng thời ghi lại Audit Log cấu hình cũ trước khi lưu.
     *
     * @param dto Đối tượng DTO chứa cấu hình giá mới.
     * @return Bảng giá đã được lưu vào CSDL (dưới dạng DTO).
     */
    @Transactional
    ...
    /**
     * Tạo một bảng giá ảo (Transient Policy) không lưu vào cơ sở dữ liệu.
     * Được sử dụng cho tính năng "Test Bảng Giá" trên giao diện quản trị, giúp Admin xem trước
     * kết quả tính tiền của bộ cấu hình vừa nhập mà không ảnh hưởng tới dữ liệu thật.
     *
     * @param dto Cấu hình bảng giá từ giao diện.
     * @return Đối tượng PricingPolicy nằm hoàn toàn trên RAM.
     */
