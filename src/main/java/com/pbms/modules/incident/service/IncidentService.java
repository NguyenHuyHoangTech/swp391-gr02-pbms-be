 ...
 * 
 * =========================================================================================
 * MỤC LỤC CHI TIẾT CÁC HÀM XỬ LÝ TRONG SERVICE:
 * 
 * 1. CÁC HÀM BẢO MẬT & TIỆN ÍCH
 * - getCurrentUser(): Trích xuất thông tin người dùng đang đăng nhập từ SecurityContext. Gọi bởi: Các hàm nội bộ. Trả về: User entity.
 * - verifyVehicleOwnership(): Đảm bảo khách hàng chỉ được báo cáo sự cố cho xe chính chủ. Gọi bởi: createIncident. Trả về: void (ném lỗi nếu gian lận).
 * - mapToDTO(): Chuyển đổi Entity sang DTO và breakdown tiền phí. Gọi bởi: Hầu hết các API trả về. Trả về: IncidentTicketDTO.
 * - saveAndBroadcast(): Lưu DB và phát WebSocket. Gọi bởi: Cuối các hàm tạo/cập nhật sự cố. Trả về: IncidentTicket.
 * 
 * 2. LUỒNG TẠO SỰ CỐ
 * - createIncident(): Khách/Bảo vệ tạo sự cố, áp dụng mức phạt tự động. Gọi bởi: API (POST /incidents). Trả về: IncidentTicket.
 * 
 * 3. LUỒNG XỬ LÝ 2 GIAI ĐOẠN (DUYỆT & THANH TOÁN)
 * - processPhase1(): Duyệt hồ sơ, chốt phạt, chờ thanh toán (Giai đoạn 1). Gọi bởi: API (PUT /incidents/{id}/process-phase1). Trả về: IncidentTicketDTO.
 * - resolveIncident(): Thu tiền, check JWT Checkout, hoàn tất sự cố (Giai đoạn 2). Gọi bởi: API (PUT /incidents/{id}/resolve). Trả về: IncidentTicketDTO.
 * 
 * 4. LUỒNG HỦY
 * - cancelIncident(): Hủy sự cố báo nhầm, rollback thẻ/phiên. Gọi bởi: API (PUT /incidents/{id}/cancel). Trả về: IncidentTicketDTO.
 * 
 * 5. LUỒNG CRONJOB QUÁ HẠN (OVERSTAY)
 * - handleOverstayVehicles(): Cronjob tự động quét xe đỗ >72h. Gọi bởi: Spring Scheduler (2:00 AM). Trả về: void.
 * - handleTimeFastForward(): Lắng nghe sự kiện tua thời gian Simulator. Gọi bởi: Spring Event. Trả về: void.
 * - hasCrossedTime(): Tính toán logic tua qua 2h sáng. Gọi bởi: handleTimeFastForward. Trả về: boolean.
 * 
 * 6. LUỒNG TRA CỨU
    ...
    public IncidentTicket createIncident(IncidentTicketRequest request, String email) {
        ...
        if (session != null) {
            if ("LPR_MISMATCH".equals(request.getIssueType()) && request.getCorrectPlateNumber() != null) {
                ...
                // Keep ticket in PENDING status so staff can add photo and explanation in Phase
                // 1
