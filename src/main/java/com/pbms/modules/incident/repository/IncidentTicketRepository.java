 ...
 * =========================================================================================
 * MỤC ĐÍCH:
 * Interface cung cấp các phương thức giao tiếp với Database (bảng
 * incident_tickets)
...
public interface IncidentTicketRepository extends JpaRepository<IncidentTicket, Long> {
    ...
    /**
     * Tìm tất cả sự cố theo loại và theo trạng thái (Ví dụ: LOST_CARD đang
     * PENDING).
     */
     ...
     * 2. HOẶC sự cố gắn với một phiên đỗ xe mà biển số xe thuộc quyền sở hữu của
     * email đó.
     * Mục đích: Cho phép khách hàng xem được sự cố của xe mình dù nhân viên là
     * người tạo hộ.
     */
    @org.springframework.data.jpa.repository.Query("SELECT DISTINCT i FROM IncidentTicket i LEFT JOIN i.session s " +
            ...
            +
            "ORDER BY i.id DESC")
    List<IncidentTicket> findAllByUserEmailOrVehicleOwner(
    ...
    /**
     * Lọc các sự cố đã được xử lý xong bởi một nhân viên cụ thể trong khoảng thời
     * gian.
     * Dùng cho nghiệp vụ tính KPI hoặc thống kê báo cáo cuối ngày của bảo vệ.
     */
    List<IncidentTicket> findByUserIdAndResolvedAtBetweenAndStatus(Long userId, java.time.LocalDateTime start,
            java.time.LocalDateTime end, String status);

    /**
     * Kiểm tra xem phiên đỗ xe này có sự cố nào cùng loại mà đang KHÔNG ở trạng
     * thái cụ thể hay không.
     */
    boolean existsBySessionIdAndIssueTypeAndStatusNot(Long sessionId, String issueType, String status);

    /**
     * Kiểm tra xem phiên đỗ xe có sự cố thuộc một trong các trạng thái đang chờ xử
     * lý hay không.
     * Dùng để chặn tạo trùng ticket (Ví dụ: Đang có ticket LOST_CARD PENDING thì
     * không cho tạo thêm).
     */
     ...
     */
    boolean existsBySessionIdAndIssueTypeInAndStatusIn(Long sessionId, java.util.List<String> issueTypes,
            java.util.List<String> statuses);

    /**
     * Kiểm tra xem phiên đỗ xe đã từng có loại sự cố này chưa (Bất kể trạng thái
     * nào).
     */
    ...
    List<IncidentTicket> findBySessionPlateAndIssueTypeAndStatus(
    ...
    /**
     * Tìm sự cố của một xe cụ thể (theo biển số + loại xe) đang ở một trạng thái
     * nhất định.
     */
     ...
     * Dùng cho Dashboard quản lý để hiển thị cảnh báo (Ví dụ: Có bao nhiêu xe máy
     * đang mất thẻ).
     */
